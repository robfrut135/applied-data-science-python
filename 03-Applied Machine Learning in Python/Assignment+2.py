...
def answer_one():
    ...
    for d in degrees:
        ...
        result[index,:] = linreg.predict(X_predict_poly).reshape(1,-1)
...
def answer_four():
    ...
    linreg = LinearRegression().fit(X_train_poly, y_train)
    r2_test_linear = r2_score(y_test, linreg.predict(X_test_poly))
       
    linlasso = Lasso(alpha=0.01, max_iter=10000).fit(X_train_poly, y_train)
    r2_test_lasso = r2_score(y_test, linlasso.predict(X_test_poly))
        
    return (r2_test_linear, r2_test_lasso)
...
def answer_five():
    ...
    df = pd.DataFrame(clf.feature_importances_, index=X_train2.columns.values, columns=["value"])        
    
    return list(df.sort_values(by="value", ascending=False).head().index.values)
answer_five()
...
def answer_seven():
    ...
    # param_range = np.logspace(-4,1,6)
    # print(param_range)
    # output> [  1.00000000e-04   1.00000000e-03   1.00000000e-02   1.00000000e-01    1.00000000e+00   1.00000000e+01]
