...
def answer_one():
    ...
    a = nx.degree_centrality(G1)[100]
    b = nx.closeness_centrality(G1)[100]
    c = nx.betweenness_centrality(G1, normalized=True, endpoints=False)[100]
    return (a, b, c)
...
def answer_four():
    ...
    return sorted(b.items(), key=lambda x: x[1], reverse=True)[0][0]
...
def answer_six():
    ...
    a = nx.pagerank(G2, alpha=0.85)
    
    return [item[0] for item in sorted(a.items(), key=lambda x: x[1], reverse=True)[:5]]
